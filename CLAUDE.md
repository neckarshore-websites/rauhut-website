...
- **`public/llms.txt` moves WITH homepage copy, same pass (Decision 12):** any content change that touches the rendered homepage's identity strings — job title, tagline, positioning, section wording that llms.txt echoes — updates `public/llms.txt` in the same commit/PR, not as a follow-up. It is a machine-reader surface nobody looks at, which is exactly why it drifts silently (see Decision 12). `scripts/public-figures.test.mjs` guards the four measured figures on this file; it does not guard prose like the job title, so a title change here needs eyes, not just green CI.
...
| 12 | 2026-09-11 c | **D12 — `public/llms.txt` updates in the same pass as homepage copy, not as a follow-up** | Founder instruction after the hero (P1) + IA (P2) passes both left `llms.txt` carrying the retired "Technical Product Owner & AI Product Builder" title — a machine-reader surface nobody visually checks, so it drifts silently. `scripts/public-figures.test.mjs` guards its four measured figures but not prose like the job title, so this is a process rule, not a test |
| 13 | 2026-09-12 d | **D13 — P11 hierarchy: amber overline stays as chapter marker, real white `h2` below it; teal = text links only; body text in `text`, not `muted`** | Founder design critique 2026-09-12: the page read as restless because the `h2`s were 12 px amber overlines (smallest text on the page) while `h3` role titles were 20 px teal — child louder than parent — and most reading text sat in the mute gray. Founder kept amber explicitly. Result: every section = amber `<p>` label + 24 px white `h2` headline; no heading carries teal; lists are real `list-disc`; stack block is a `<dl>`; hairline raised `#262626` → `#333333`; hero 6 stacked lines → 4; rail/burger list all five sections. `/stylesheet` (footer-only, `nofollow`, noindex header + meta, not in sitemap) renders the live tokens as a reference. Content trim of Projekte (critique item 5) and Impressum/Datenschutz/404 alignment are deferred, see BACKLOG.md |
